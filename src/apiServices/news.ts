...
export const getHighImpactNews = async (selectedTag: string, limit: number) => {
  const client = await apiClient();
  console.log("Selected Limit in API:", limit);
  try {
    const response = await client.get(
      `/news/high-impact?threshold=6.5&limit=${limit}&category=${selectedTag}`
    );
    // const response = await client.get(
    //   `/news/high-impact?threshold=6.5&limit=20&category=${selectedTag}`
    // );
    return response;
  } catch (error) {
    throw error;
...
//Get High-Impact News with newsId
...
export const getHighImpactNewsByFilter = async (
  filterDays: any,
  limit: any
) => {
  const client = await apiClient();
  ...
  try {
    const response = await client.get(
      `/news/high-impact/timerange/${filterDays}?limit=${limit}`
    );
    return response;
  } catch (error) {
    throw error;
